...
const POWERUPS = [
    { type: "heart", label: "♥", weight: 50, title: "Vida" },
    { type: "fast", label: "+", weight: 20, title: "Ráfaga" },
    { type: "mirror", label: "◇", weight: 30, title: "Espejo" }
];
...
function spawnEnemy(timestamp) {
    ...
    lastSpawn = timestamp;
    const isCorrect = Math.random() < 0.808;
...
function spawnPowerup(timestamp) {
    ...
    element.innerHTML = `${data.label}<small>${data.title}</small>`;
